...
public class EarthquakeCityMap extends PApplet {
	...
	private void sortAndPrint(int numToPrint) {
		EarthquakeMarker[] earthquake = quakeMarkers.toArray(new EarthquakeMarker[0]);
		Arrays.sort(earthquake);
		if(quakeMarkers.size()<numToPrint) {
			for(EarthquakeMarker eq: earthquake) {
			...
			for(int i=0;i<numToPrint;i++) {
				System.out.println(earthquake[i]);
			}
			return;
		}
